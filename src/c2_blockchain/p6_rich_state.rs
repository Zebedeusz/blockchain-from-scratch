...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        ...
        for i in 0..chain.len() {
            let next_header = chain.get(i).unwrap().clone();
...
impl Block {
    ...
    pub fn genesis(genesis_state: &State) -> Self {
        let state_root = hash(&genesis_state);
    ...
    pub fn verify_sub_chain(&self, pre_state: &State, chain: &[Block]) -> bool {
        ...
        for i in 0..chain.len() {
            ...
            for extr in &next_block.body {
                curr_state.sum += extr;
                curr_state.product *= extr;
            }
...
fn build_invalid_child_block_with_valid_header(parent: &Header, pre_state: &State) -> Block {
    ...
    let mut child = parent_block.child(pre_state, vec![1, 2, 3]);
    child.body = vec![4, 5, 6, 7];
